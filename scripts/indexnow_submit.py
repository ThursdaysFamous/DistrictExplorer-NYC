...
Ownership is proven by hosting the key as a plain-text file at the site root:
    ...
    https://nyc.chidistricts.com/4505524963614a30b058bc3bb209c0a5.txt

The key is a PUBLIC ownership token, not a secret — publishing it is the whole
point of the protocol, so it lives in the repo and deploys with the site. It is
per-host: nyc.chidistricts.com has its own key, distinct from the apex fork.

Usage (run only AFTER a deploy where the key file is already live):

    python3 scripts/indexnow_submit.py                              # submit the homepage
    python3 scripts/indexnow_submit.py https://nyc.chidistricts.com/ https://nyc.chidistricts.com/other
...
KEY = "4505524963614a30b058bc3bb209c0a5"
HOST = "nyc.chidistricts.com"
